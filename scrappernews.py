...
# Configuration
TOKEN = '6656925028:AAHRGQec30IpWCPMzZh2FuQRwMb4Ah46WQY'
CHAT_ID = '1309504461'
GROUP_INTERVAL = 150
...
#     colors = ['#1a2636', '#0f6f5f', '#39a789', '#63e1b4']
...
def detect_cancellations(previous_book, current_book):
    def compare_orders(prev, curr, side):
        ...
        for _, row in cancellations.iterrows():
            ...
            send_telegram_notification(f"{color_square} Large {side} cancellation detected: {cancelled_amount:.4f} coins at ${row['price']}")
...
class OrderBookGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # Set the window icon
        self.setWindowIcon(QIcon(r'C:\Users\danie\liquidation\icon.png'))
        
        self.setup_ui()
        self.set_default_settings("BTCUSDT")
        self.update_signal.connect(self.update_order_book)
        self.log_signal.connect(self.update_log)  # Connect the new signal
        self.start_update_thread()

    def update_log(self, message):
        self.log_output.append(message)

    def set_default_settings(self, pair):
        if pair in default_settings:
            ...
            self.update_group_interval(settings["group_interval"])
            self.update_large_wall_threshold(settings["large_wall_threshold"])
            self.update_cancellation_threshold(settings["cancellation_threshold"])
        else:
            print(f"No default settings found for pair: {pair}")
    ...
    def setup_ui(self):
        ...
        self.setStyleSheet("""
            ...
            QPushButton {
                ...
                border: none;
                padding: 5px;
                border-radius: 5px;  /* This makes the buttons round */
                min-width: 20px;  /* Ensure buttons are at least 30px wide */
                min-height: 20px;  /* Ensure buttons are at least 30px tall */
            }
            QPushButton:hover {
                background-color: #FFE14D;  /* Lighter color on hover */
            }
            QTableWidget {
        ...
        self.pair_dropdown.currentTextChanged.connect(self.update_pair)
        control_layout.addWidget(self.pair_dropdown)

        self.group_interval_input = QSpinBox()
        self.group_interval_input.setRange(10, 1000)
        self.group_interval_input.setValue(GROUP_INTERVAL)
        self.group_interval_input.valueChanged.connect(self.update_group_interval)
        control_layout.addWidget(QLabel("Group Interval:"))
        control_layout.addWidget(self.group_interval_input)

        # Threshold inputs
        self.large_wall_input = QDoubleSpinBox()
        self.large_wall_input.setRange(0, 1000000)
        self.large_wall_input.setValue(LARGE_WALL_THRESHOLD)
        self.large_wall_input.valueChanged.connect(self.update_large_wall_threshold)
        control_layout.addWidget(QLabel("Large Wall:"))
        control_layout.addWidget(self.large_wall_input)

        self.cancellation_input = QDoubleSpinBox()
        self.cancellation_input.setRange(0, 1000000)
        self.cancellation_input.setValue(CANCELLATION_THRESHOLD)
        self.cancellation_input.valueChanged.connect(self.update_cancellation_threshold)
        control_layout.addWidget(QLabel("Cancellation:"))
        control_layout.addWidget(self.cancellation_input)

        self.cooldown_input = QSpinBox()
        self.cooldown_input.setRange(0, 86400)
        self.cooldown_input.setValue(NOTIFICATION_COOLDOWN)
        self.cooldown_input.valueChanged.connect(self.update_notification_cooldown)
        control_layout.addWidget(QLabel("Cooldown (s):"))
        control_layout.addWidget(self.cooldown_input)
    ...
    def set_column_widths(self):
        # Set fixed width for Price and Amount columns
        self.order_book_table.setColumnWidth(0, 100)  # Price column
        self.order_book_table.setColumnWidth(1, 100)  # Amount column

    def toggle_log_output(self):
        if self.log_output.isVisible():
            self.log_output.hide()
            self.toggle_log_button.setText("Show Log")
            self.resize(self.width() - self.log_output.width(), self.height())
        else:
        ...
        while True:
            if is_running.is_set():
                try:
                    ...
                    bids, asks = analyze_order_book(current_order_book, self)  # Pass 'self' here
                    if bids is None or asks is None:
                        raise ValueError("Failed to analyze order book data")
    ...
    def update_pair(self, new_pair):
        ...
        if new_pair != current_symbol:
            ...
            # Set the default settings for the new pair
            self.set_default_settings(current_symbol)
    ...
    def update_order_book(self, data):
            ...
            # Creating custom color maps for bids (green) and asks (red)
            ...
            for i, ask in enumerate(reversed(asks)):
                ...
                self.set_table_item(i, 0, f"{ask[0]:.2f}", QColor("#F6465D"), color)  # Red text for asks
            ...
            for i, bid in enumerate(bids):
                ...
                self.set_table_item(row, 0, f"{bid[0]:.2f}", QColor("#2EBD85"), color)  # Green text for bids
                self.set_table_item(row, 1, f"{bid[1]:.4f}", QColor("#2EBD85"), color)

            #self.log_output.append("Order book updated successfully.")
    ...
    def update_large_wall_threshold(self, value):
        global LARGE_WALL_THRESHOLD
        LARGE_WALL_THRESHOLD = value
        send_telegram_notification(f"Large wall threshold set to {value}")
        self.log_output.append(f"Large wall threshold set to {value}")

    def update_cancellation_threshold(self, value):
        global CANCELLATION_THRESHOLD
        CANCELLATION_THRESHOLD = value
        send_telegram_notification(f"Cancellation threshold set to {value}")
        self.log_output.append(f"Cancellation threshold set to {value}")

    def update_notification_cooldown(self, value):
        global NOTIFICATION_COOLDOWN
        NOTIFICATION_COOLDOWN = value
        send_telegram_notification(f"Notification cooldown set to {value} seconds")
        self.log_output.append(f"Notification cooldown set to {value} seconds")
